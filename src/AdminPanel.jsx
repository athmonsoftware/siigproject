    ...
    <main className="min-h-screen bg-slate-950 px-5 py-16 text-white">
      <div className="mx-auto max-w-2xl border border-white/15 bg-white/5 p-8 shadow-2xl">
        ...
        <p className="mt-4 leading-7 text-slate-300">
          ...
          <code className="ml-1 bg-white/10 px-1.5 py-1">.env.local</code>, add
          the project URL and public anon key, then run the database setup file.
        </p>
        <a
          href="/"
          className="mt-8 inline-flex min-h-11 items-center bg-white px-5 font-bold text-slate-950"
        >
          Return to website
        </a>
      </div>
      ...
      <section className="hidden bg-emerald-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div>
          ...
          <h1 className="mt-5 max-w-lg text-5xl font-black leading-tight">
            Manage the SIIG website and every new enquiry.
          </h1>
          <p className="mt-5 max-w-md text-lg leading-8 text-emerald-100/75">
            Content changes publish to the public website without editing code.
          </p>
        </div>
        <p className="text-sm text-emerald-100/60">
          Safety Innovations Impact Group
        </p>
      </section>
      <section className="flex items-center justify-center px-5 py-12">
        <form
          onSubmit={recoveryMode ? requestReset : submit}
        ...
        >
          ...
          <p className="mt-2 text-slate-600">
            {recoveryMode
              ? "We will email a secure reset link to your administrator address."
              : "Sign in with your approved SIIG administrator account."}
          </p>
          ...
          <button
            disabled={loading}
            className="mt-6 flex min-h-12 w-full items-center justify-center bg-emerald-800 px-5 font-bold text-white transition hover:bg-emerald-900 disabled:opacity-60"
          >
            {loading
              ? "Working…"
              : recoveryMode
          ...
          <button
            ...
            className="mt-4 w-full text-center text-sm font-bold text-emerald-800 hover:underline"
          >
            {recoveryMode ? "Return to sign in" : "Forgot password?"}
          </button>
          <a
            href="/"
            className="mt-6 block text-center text-sm font-semibold text-slate-600 hover:text-emerald-800"
          >
            View public website
          </a>
        </form>
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-950 px-5">
      ...
      >
        ...
        <p className="mt-2 text-slate-600">
          Use at least 12 characters and keep it private.
        </p>
        ...
        {message && (
          <p
            role="status"
            className="mt-4 bg-slate-100 p-3 text-sm text-slate-700"
          >
            {message}
          </p>
        )}
        <button
          disabled={loading}
          className="mt-6 min-h-12 w-full bg-emerald-800 px-5 font-bold text-white hover:bg-emerald-900 disabled:opacity-60"
        >
...
function Overview({ enquiries, content }) {
  ...
  return (
    <div>
      <Header
        title="Overview"
        subtitle="A clear view of website activity and content status."
      />
...
function ContentEditor({ rows, onSaved, userId }) {
  const [selected, setSelected] = useState("hero");
  const row = rows[selected] || {
    content: defaultContent[selected],
    is_published: true,
  };
  ...
  useEffect(() => {
    ...
    setSaved(false);
  }, [selected, row.content, row.is_published]);
  const save = async () => {
    setSaving(true);
    setSaved(false);
  ...
  return (
    <div>
      <Header
        ...
        subtitle="Edit the public copy and control what is published."
      />
      <div className="grid gap-6 lg:grid-cols-[15rem_1fr]">
        <nav
          className="h-fit border border-slate-200 bg-white p-2 shadow-sm"
        ...
        <section className="border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 grid gap-5">
            {Object.entries(draft).map(([key, value]) => (
              <label
                key={key}
                className="block text-sm font-bold capitalize text-slate-700"
              >
                {key.replaceAll("_", " ")}
...
function Enquiries({ items, onUpdated }) {
  ...
  return (
    <div>
      <Header
        title="Enquiries"
        subtitle="Review and track every message submitted through the website."
      />
...
function MediaLibrary() {
  ...
  return (
    <div>
      <Header
        title="Media library"
        subtitle="Upload and manage images for the SIIG website."
      />
      <section className="border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              JPG, PNG, WebP, GIF, or SVG. Maximum 5 MB.
            </p>
          </div>
        ...
        {message && (
          <p
            role="status"
            className="mt-4 bg-slate-100 p-3 text-sm font-semibold text-slate-700"
          >
            {message}
          </p>
        )}
...
function SettingsPanel({ user }) {
  return (
    <div>
      <Header
        title="Settings"
        subtitle="Account and integration details for this workspace."
      />
...
export default function AdminPanel() {
  ...
  if (window.location.pathname === "/admin/reset-password" && session)
